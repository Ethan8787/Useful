...
public class ServerStopCommand implements NontageCommand {
    ...
    public void execute(CommandSender sender, String label, String[] args) {
        ...
        if (args.length == 0) {
            Bukkit.broadcastMessage(Messages.PREFIX + "§c伺服器即將關閉");
            startStopCountdown(5);
            return;
        }

        if (args.length != 2) {
            sender.sendMessage(Messages.PREFIX + "§c用法：/serverstop <yyyy/MM/dd> <HH:mm:ss>");
            sender.sendMessage(Messages.PREFIX + "§c例如：/serverstop 2026/02/21 10:32:45");
            return;
        }
        ...
        taskId = Bukkit.getScheduler().runTaskLater(
                Main.getInstance(),
                () -> {
                    Bukkit.broadcastMessage(Messages.PREFIX + "§c伺服器排程關閉時間到，準備關服");
                    startStopCountdown(5);
                },
                delayTicks
        ).getTaskId();
